...
The issuer **SHALL** perform a biometric verification of the applicant to the biometric data records obtained from either the PIV Card[^OCCorBIO] or PIV enrollment record. Minimum accuracy requirements for
...
[^OCCorBIO]: BIO-A or OCC-AUTH authentication mechanism is performed when using the PIV Card for biometric  verification. 
...
: When PIN reset is performed in person at the issuing facility, before providing the reset PIV Card
    ...
    [^OCCorBIOv2]: BIO-A or OCC-AUTH authentication mechanism is performed when using the PIV Card for biometric verification. 
...
: PIN reset at a supervised remote identity proofing station combines the assurance
    ...
    records stored either on the PIV Card[^OCCorBIOv3] or in the PIV enrollment record. In
    ...
    photograph printed on the PIV Card. 
     ...
     [^OCCorBIOv3]: BIO-A or OCC-AUTH authentication mechanism is performed when using the PIV Card for biometric verification. 
