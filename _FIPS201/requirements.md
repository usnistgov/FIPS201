...
Derived PIV credentials are additional PIV credentials that are issued based on proof of possession and control of a PIV Card. These credentials are not embedded in the PIV Card but instead are stand-alone or integrated in a variety of devices/platforms. Derived PIV credentials play an important role for environments where use of the PIV Card is not easily supported.
